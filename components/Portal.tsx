...
const Portal: React.FC<PortalProps> = ({ onNavigate }) => {
    ...
    const FlashCard = ({ title, icon: Icon, children, accent = "brand-gold" }: any) => (
        <div className="bg-brand-950/80 border border-brand-gold/10 p-10 rounded-[3rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] relative overflow-hidden group hover:border-brand-gold/30 transition-all duration-700">
    ...
    const UnifiedDashboard = () => (
        <div className="animate-fade-in space-y-12 pb-32">
            ...
            <div className="relative p-16 rounded-[4rem] border border-brand-gold/20 shadow-[0_0_100px_rgba(212,175,55,0.1)] overflow-hidden bg-brand-950/50">
                ...
                <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-16">
                    <div className="max-w-3xl">
                        ...
                        <p className="text-white/50 text-2xl font-light leading-relaxed mb-12">
                        ...
                        <div className="flex gap-6">
                            <div className="p-4 bg-brand-gold/5 border border-brand-gold/10 rounded-2xl flex items-center gap-4">
                            ...
                            <div className="p-4 bg-brand-terminal/5 border border-brand-terminal/10 rounded-2xl flex items-center gap-4">
                    ...
                    <div className="shrink-0 relative group">
                        ...
                        <div className="w-56 h-56 rounded-[3rem] border-2 border-brand-gold/30 p-2 flex items-center justify-center relative bg-brand-950 overflow-hidden shadow-2xl">
                             <img 
                                src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=300&auto=format&fit=crop" 
                                className="w-full h-full rounded-[2.5rem] object-cover grayscale brightness-75 group-hover:grayscale-0 transition-all duration-1000"
                                alt="Access Node"
                            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                <FlashCard title="Sovereign Corridors" icon={Globe}>
                    <div className="space-y-4">
                        {[
                            ...
                            <div key={i} className="flex justify-between items-center p-6 bg-brand-gold/5 rounded-3xl border border-brand-gold/10 hover:border-brand-gold transition-all cursor-pointer group/node">
                                <div>
                ...
                <FlashCard title="Strategic Intelligence" icon={Cpu}>
                    <p className="text-white/40 text-sm mb-10 leading-relaxed font-mono">
                ...
                <FlashCard title="Market Velocity" icon={TrendingUp}>
                    <div className="space-y-6">
                        <div className="flex justify-between items-end">
                            <span className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Asset Utilization</span>
                            <span className="text-brand-gold font-bold text-lg">84%</span>
                        </div>
                        <div className="w-full h-2 bg-brand-950 rounded-full overflow-hidden border border-brand-gold/10">
                            <div className="h-full bg-brand-gold shadow-[0_0_10px_#d4af37] w-[84%]"></div>
                        </div>
                        <p className="text-xs text-white/40 leading-relaxed italic">Global market sentiment indicates consistent institutional top-quartile performance.</p>
                    </div>
                </FlashCard>
            </div>

            {/* AI Reasoning Section - Previously gate-kept */}
            <div className="bg-brand-950/80 border border-brand-gold/10 p-16 rounded-[4rem] shadow-2xl relative">
                <div className="scanline-overlay opacity-5"></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                ...
                <FlashCard title="AI Core Status" icon={Cpu} accent="brand-terminal">
                    <div className="p-8 bg-brand-950 rounded-[2.5rem] border border-brand-terminal/10 space-y-6 relative overflow-hidden">
                ...
                <FlashCard title="System Node Triggers" icon={Wrench}>
                    <div className="grid grid-cols-2 gap-4">
                        {['Market', 'Compliance', 'Finance', 'Venture'].map(btn => (
                            <button key={btn} onClick={() => onNavigate(`/data-tools/${btn.toLowerCase()}`)} className="p-4 bg-brand-950 border border-brand-gold/10 rounded-2xl text-[10px] font-mono font-bold text-white uppercase hover:border-brand-gold hover:bg-brand-gold/5 transition-all shadow-inner tracking-widest">{btn}_ACCESS</button>
                        ))}
                    </div>
                </FlashCard>
            </div>
            ...
            <button 
                ...
                className="w-full py-10 border-2 border-dashed border-brand-gold/20 text-brand-gold/60 hover:text-brand-gold hover:border-brand-gold hover:bg-brand-gold/5 rounded-[3rem] transition-all text-sm font-extrabold uppercase tracking-[0.5em] flex items-center justify-center gap-6 shadow-inner group"
            >
    ...
    return (
        <div className="min-h-screen bg-brand-950 pt-32 pb-12 relative overflow-hidden">
            ...
            <div className="absolute inset-0 z-0 pointer-events-none">
                <div className="absolute top-[-10%] right-[-10%] w-[1000px] h-[1000px] bg-brand-gold/5 rounded-full blur-[200px] animate-glow"></div>
                <div className="absolute bottom-[-10%] left-[-10%] w-[1000px] h-[1000px] bg-brand-gold/5 rounded-full blur-[200px] animate-glow delay-1000"></div>
                <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.08] mix-blend-overlay"></div>
            </div>

            {isAuthenticating && (
                <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-brand-950/95 backdrop-blur-3xl animate-fade-in">
                    <div className="scanline-overlay"></div>
                    ...
                    <div className="w-full max-w-md space-y-2 text-left px-12">
                        {terminalLines.map((line, i) => (
                            <p key={i} className="terminal-text text-[11px] animate-fade-in uppercase tracking-[0.2em]">{line}</p>
                        ))}
                    </div>
                    <div className="mt-16 w-80 h-1 bg-brand-gold/10 rounded-full overflow-hidden border border-brand-gold/20">
                        <div className="h-full bg-brand-gold animate-shimmer shadow-[0_0_20px_#d4af37]"></div>
                    </div>
                    <p className="mt-8 text-brand-gold font-bold uppercase tracking-[0.6em] text-[10px] animate-pulse">AUTHORIZING_IDENTITY...</p>
                </div>
            )}

            {!isAuthorized ? (
                <div className="max-w-xl mx-auto px-6 relative z-10 pt-12">
                    <div className="bg-brand-950/80 border border-brand-gold/20 rounded-[4rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.8)] p-16 text-center backdrop-blur-3xl animate-fade-up relative overflow-hidden">
                        <div className="scanline-overlay opacity-5"></div>
                        ...
                        <p className="text-white/40 text-lg mb-16 font-medium tracking-tight">Bypass role-restrictions. Access Federgreen Global Node.</p>
                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10">
                    <div className="flex flex-col md:flex-row justify-between items-center mb-20 gap-10 bg-brand-950/50 p-8 rounded-[3rem] border border-brand-gold/10 backdrop-blur-xl">
                        ...
                        <div className="flex items-center gap-6">
                            <div className="hidden md:flex flex-col items-end">
                                <p className="text-[9px] text-white/30 font-bold uppercase tracking-widest">System Latency</p>
                                <p className="text-brand-terminal font-mono text-xs">0.02ms</p>
                            </div>
                            <button className="w-16 h-16 rounded-[1.5rem] bg-brand-gold/5 border border-brand-gold/10 flex items-center justify-center text-white/40 hover:text-brand-gold hover:border-brand-gold transition-all relative group shadow-inner">
                                <Bell size={24} />
