...
const Footer: React.FC<FooterProps> = ({ onNavigate, footerLinks }) => {
    ...
    const renderLinkGroup = (group: FooterLinkGroup) => (
        <div>
            <h3 className="text-white font-black mb-10 uppercase tracking-[0.4em] text-[10px] border-b border-brand-gold/20 pb-4 inline-block">{group.title}</h3>
            <ul className="space-y-6 text-sm font-bold uppercase tracking-widest">
                {group.links.map(link => (
                    <li key={link.path}>
                        <button onClick={() => onNavigate(link.path)} className="hover:text-brand-gold transition-all hover:translate-x-2 flex items-center gap-3 group">
                            {link.label}
        ...
        <footer className="relative bg-brand-950 text-slate-400 pt-32 pb-12 border-t border-brand-800/50 overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-20 mb-32">
                    ...
                    {renderLinkGroup(footerLinks.access)}
                ...
                <div className="pt-12 border-t border-brand-800/50 flex flex-col lg:flex-row justify-between items-center text-[10px] text-white/20 font-black uppercase tracking-[0.5em] gap-8">
                    ...
                    <button onClick={scrollToTop} className="mt-8 lg:mt-0 p-5 rounded-2xl bg-brand-gold text-brand-950 hover:bg-white transition-all hover:-translate-y-2 shadow-2xl">
                        <ArrowUp size={20}/>
                    </button>
                </div>
            </div>
